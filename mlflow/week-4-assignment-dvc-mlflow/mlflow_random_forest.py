...
test_size = rf_params.pop('test_size')
random_state = rf_params.pop('random_state')
sales_threshold = rf_params.pop('sales_threshold')
...
with mlflow.start_run(run_name="Random Forest Classifier model"):
    ...
    model_uri = f"runs:/{mlflow.active_run().info.run_id}/random_forest_classifier_model"
    mlflow.register_model(model_uri=model_uri, name="AdvertisingSalesClassificationRFModel")
    print(f"Model registered in MLflow under name 'AdvertisingSalesClassificationRFModel'")